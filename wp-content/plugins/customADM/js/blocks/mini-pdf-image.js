...
wp.blocks.registerBlockType('cms-adm/mini-pdf', {
	title: 'PDF',		// Block name visible to user
		...
		for(index=0;index<sizelines;index++){
			...
			lines_editor.push(
				...
				el('div',
					...
					el('a',
						{
							className:"d-flex mx-auto height-square w-100 d-block overflow-hidden align-items-center",
...
try{
				if(props.attributes.url[index]!=""){
					dataImagefirstPagePDFbyURL(index,props.attributes.url[index])
					setTimeout(function(){
						var acc_dataImage= [...props.attributes.dataImage];
						acc_dataImage_postion=acc_dataImage[index];
						acc_dataImage[index]="";
						props.setAttributes( { dataImage: acc_dataImage } );
						acc_dataImage[index]=acc_dataImage_postion	;
						props.setAttributes( { dataImage: acc_dataImage } );
					},100);
				}
			}catch(e){};
