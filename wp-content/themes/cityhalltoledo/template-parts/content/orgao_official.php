...
?>
    <div class="container-block-by">
        <?php 
        for($count=0;$count<(min(4,count($resultado)));$count++) { 
            $valor=$resultado[$count];?>
            <div class="block-by-4 d-flex flex-wrap p-1 justify-content-center" style="max-width:500px;min-width:280px">
                <a target="_blank" href="<?php echo wp_get_attachment_url($valor->media_id); ?>"><canvas class="m-2 overflow-hidden canvas-pdf-image p-0" url="<?php echo wp_get_attachment_url($valor->media_id); ?>"  
                style="width:auto;height:240px;min-width:240px;max-width:100%;border:black solid;">
                </canvas>
                </a>
    ...
    <div class="container">
        <br>
        <form method="POST"><div class="input-group mb-4 mx-auto" style="width: 400px;min-width:260px;">
            ...
            <div class="row">
                <div class="mr-1 col " style="overflow-x:hidden">
                <?php 
                    if(count($resultado)>4 )
                    for($count=4;$count<(min(14,count($resultado)));$count++) { 
                        $valor=$resultado[$count];    
                    ?>
                        <div class="mr-1 w-100 d-flex border rounded border-dark <?php if($count!=4) echo "border-top-0";?>" >
                            <label class="p-2 w-100 overflow-hidden d-flex" style=" word-wrap:normal ;height:42px;line-height:30px;">
                                <a target="_blank" href="<?php echo wp_get_attachment_url($valor->media_id); ?>" class="text-decoration-none d-flex color-1" style=" word-wrap: break-word;overflow-x:overflow; width:1000px;"><?php echo $valor->title; ?></a>
                            </label>
                ...
                <div class="ml-1 col " style="overflow-x:hidden">
                <?php 
                    if(count($resultado)>14 )
                    for($count=14;$count<(min(24,count($resultado)));$count++) { 
                        $valor=$resultado[$count];    
                    ?>
                        <div class="d-flex  w-100 border rounded border-dark <?php if($count!=14) echo "border-top-0";?>" >
                            <label class="p-2 w-100 overflow-hidden d-flex" style=" word-wrap:normal ;height:42px;line-height:30px;">
                            <a target="_blank" href="<?php echo wp_get_attachment_url($valor->media_id); ?>" class="text-decoration-none d-flex color-1" style=" word-wrap: break-word;overflow-x:overflow; width:1000px;"><?php echo $valor->title; ?></a>
                            </label>
                        </div>
