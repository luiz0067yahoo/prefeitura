<div href="#<?php echo $valor->nome_do_evento;?>" class="<?php echo $classNameEventBanner;?> height-square p-2 mx-auto color-1 pe-auto"  onclick="{jQuery('#event-<?php echo $valor->id;?>').toggleClass('d-none');loadDimension;setTimeout(loadDimension,100);setTimeout(loadDimension,200);}" style="overflow-x:hidden"> 
	...
	?>
	<div id="viewImg" class="containerScheduleEvent w-100 height-square <?php  echo $classCSSName;?>" style="background-image:url('<?php echo  wp_get_attachment_image_url($valor->media_id,'large',false );?>');background-position-x:0<?php if(isset($valor->posicao))echo (100-$valor->posicao);?>%;"> 
...
<?php if (isset($valor->categoria_evento) && !empty($valor->categoria_evento)){?>
						<p class="category">
							<?php echo "#".$valor->categoria_evento;?>
						</p>
<?php }?>
						<p class="event-title" >
						<?php 
